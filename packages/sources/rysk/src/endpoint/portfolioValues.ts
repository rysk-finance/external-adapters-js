...
function populateEventMaps(
  ...
  contractsState.vaultLiquidationRegisteredEvents.forEach((x) => {
    ...
    const newAmounts: OptionEventAmounts = {
      amount: amountLiquidated,
      escrow: collateralLiquidated,
    }
...
async function enrichOptionPositions(
  ...
  const enrichedOptionPositions = optionPositions.map(async (x) => {
    ...
    let timeToExpiration = genOptionTimeFromUnix(
      Number(timestamp),
    ...
    if (expired) {
      const [price] = await opynOracle.getExpiryPrice(
        contractsState.underlying,
        seriesInfo.expiration,
      )
      timeToExpiration = 0
...
async function getContractsState(
  ...
  const optionsContractSettledEvents = await optionRegistry.queryFilter(
    optionsContractSettledFilter,
  )
...
function computeDelta(
  ...
  const externalDeltaNumber = Number(fromWei(externalDelta))
...
async function getUtilizationCurve(liquidityPool: LiquidityPool): Promise<UtilizationCurve> {
  ...
  const utilizationCurve: UtilizationCurve = {
